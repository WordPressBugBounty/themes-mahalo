...
class Mahalo_Walkernav extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if( isset( $args->has_children ) && !$args->has_children && ( ( $child_cats && $hasMegaMenu_subcat ) || ( $hasMegaMenu && $mm_posts_query ) && $depth == 0 ) ) {
            ...
            $item_output .= '<span class="icon">'.mahalo_theme_svg('chevron-down',true).'</span>';
        ...
        $item_output .= ( ( $depth == 0 || 1 ) && $has_children ) ? '<span class="icon">'.mahalo_theme_svg('chevron-down',true).'</span></a>' : '</a>';
            ...
            if( ( $child_cats && $hasMegaMenu_subcat ) || ( $hasMegaMenu && $mm_posts_query ) ){
                ...
                if( $hasMegaMenu_subcat && $child_cats ){
                    ...
                    foreach( $child_cats as $child_cat ){
                        ...
                        $output .= '<span class="icon">'.mahalo_theme_svg('chevron-right',true).'</span>';
                        $output .= '</a>';
                        $output .= '</div>';
                ...
                if( $hasMegaMenu && $mm_posts_query ){
                    ...
                    foreach( $mm_posts_query as $mm_posts_content ){
                        ...
                        if( $mm_posts_content->ID ){
                            ...
                            $thumb_url = get_the_post_thumbnail_url($mm_posts_content->ID,'medium');
                            $post_date = get_the_date(get_option( 'date_format' ), $mm_posts_content->ID);
                        }
                        $output .= '<div class="megamenu-recent-article">';
                        $output .= '<div class="megamenu-article-image"><div class="img-hover-panel"><div class="data-bg data-bg-thumbnail data-bg-overlay img-hover-scale" data-background="'.wp_kses_post($thumb_url). '"><a class="img-link" href="'.esc_url( $url ).'"></a></div></div></div>';
                        $output .= '<div class="megamenu-article-body">';
                        $output .= '<div class="megamenu-article-date mt-10">'.wp_kses_post($post_date).'</div>';
                        $output .= '<h3 class="entry-title entry-title-xsmall"><a href="'.esc_url( $url ).'">';
